...
export function usePaginatedCollection(collectionName: string, pageSize = 10, constraints: QueryConstraint[] = []) {
  ...
  const loadMore = async () => {
    ...
    setLoading(true)
    try {
      const q = query(collection(db, collectionName), ...constraints, startAfter(lastVisible), limit(pageSize))
...
export function useDeleteDocument(collectionName: string) {
  ...
  return useMutation({
    ...
    onSuccess: (id) => {
      queryClient.invalidateQueries({ queryKey: ["document", collectionName, id] })
      queryClient.invalidateQueries({ queryKey: ["collection", collectionName] })
    },
  })
}
