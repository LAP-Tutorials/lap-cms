...
export function AnalyticsOverview() {
  ...
  const totalUsers = data.timeline.reduce(
    (acc: number, curr: any) => acc + curr.users,
    0,
  );
  const totalViews = data.timeline.reduce(
        ...
        <div className="bg-[#0a0a0a] border border-neutral-800 p-3 text-sm rounded-none shadow-xl">
          ...
          <div className="flex flex-col gap-2">
            ...
            >
              ...
              />
              {payload.find((p: any) => p.dataKey === "views")?.value} Page
              Views
            </span>
          </div>
        </div>
    ...
    <Card className="border border-neutral-800 bg-transparent text-white rounded-none">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 border-b border-neutral-800">
        ...
        <Select value={days} onValueChange={setDays}>
          ...
          <SelectContent
            className="bg-[#0a0a0a] border-neutral-800 text-white rounded-none z-[9999]"
            position="popper"
          >
            <SelectItem
              value="7"
              className="focus:bg-neutral-800 focus:text-white cursor-pointer rounded-none"
            >
              Last 7 days
            </SelectItem>
            <SelectItem
      ...
      <div className="border-t border-neutral-800">
        <Button
          asChild
          className="w-full bg-transparent hover:bg-neutral-900 text-white border-0 rounded-none h-12"
          variant="outline"
        >
          <Link
            href="/analytics"
